...
void timque(struct nhanvien *a, int n);
void dem(struct nhanvien *a, int n);
void Xuat(struct nhanvien *a,int n);
void sapxep(struct nhanvien *a, int n);
void xoa_nv(struct nhanvien *a,int &n);
void them_nv(struct nhanvien *a, int &n);
void outprintf(struct nhanvien *a,int n,FILE *f);
void tinhluong(struct nhanvien *a,int n);
void sua_thong_tin(struct nhanvien *a, int &n);
void menu();
int main()
{
	gotoXY(5,3);
...
void outprintf(struct nhanvien *a,int n,FILE *f){
	getchar();
	char filename[30];
	printf("Nhap ten file ban muon luu: \n");
	gets(filename);
    f= fopen (filename,"ab");
    if(f==NULL){
...
void menu(){
	...
	while(flat==1){
		...
		printf("\t\t7. Sua thong tin nhan vi�n.\n");
		printf("\t\t8. Thoat.");
		ve_tuong();
		printf("\n\t\t\t ==>  ");
		scanf("%d", &chon);
		switch(chon){
			case 1: 
				...
				scanf ("%d", &n);
				a=(struct nhanvien*)malloc(n*sizeof(nhanvien));
				Nhap(a,n);
				printf("\n\n\n");
				break;
			case 2:
